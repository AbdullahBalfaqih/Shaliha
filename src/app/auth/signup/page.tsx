...
export default function SignupPage() {
  ...
  const handleDetailsSubmit = async (e: React.FormEvent) => {
    ...
    if (phone.length !== 9 || !validPrefixes.includes(phonePrefix)) {
        toast({
            ...
            description: "يجب أن يكون رقم الهاتف مكونًا من 9 أرقام ويبدأ بأحد الأرقام التالية: 70, 71, 73, 77, 78.",
            variant: "destructive",
        });
        setIsLoading(false);
        return;
    ...
    if (userType === 'host') {
        ...
        try {
            ...
            if (response.success) {
                ...
                toast({
                    ...
                    description: "لقد أرسلنا رمزًا إلى بريدك الإلكتروني. الرجاء التحقق منه.",
                });
            } else {
                 toast({
        ...
        await signUp({ phone, password, fullName: `${firstName} ${lastName}`, role: 'user' });
    }
    setIsLoading(false);
  };
  ...
  const handleOtpSubmit = async (e: React.FormEvent) => {
    ...
    if (isVerified) {
        await signUp({ phone, password, fullName: `${firstName} ${lastName}`, role: userType, email: email });
    } else {
        toast({
                                        ...
                                        <Label htmlFor="phone">رقم الهاتف</Label>
                                         <div className="flex items-center gap-2" dir="ltr">
                                            ...
                                            <div className="flex items-center p-2 bg-secondary rounded-md">
                                               <Image src="https://flagcdn.com/w20/ye.png" width={20} height={15} alt="Yemen Flag" />
                                               <span className="ml-2 text-sm text-muted-foreground">+967</span>
                                            </div>
                                        </div>
                                    </div>
